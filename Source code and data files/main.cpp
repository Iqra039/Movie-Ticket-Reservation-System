...
int showTime[100];                      //an array to store showtime
...
/* -------------Function To save movies history to file history.txt-----------*/
void saveTohistory(string msg) {
	ofstream file("history.txt", ios::app);
	file << msg << endl;
	file.close();
}
/*------------------- Function to save a Movie whenever it's added----------------------*/
void saveMovieToFile(string title, string genre, int time) {
	ofstream file("movies.txt", ios::app); // append mode
	file << title << "\n" << genre << "\n" << time << "\n";
	file.close();
}
/* -------------Function to load Movies from movies.txt when the program starts------------*/
void loadMovies() {
...
void addMovie(string* titles, string* genres, int* times, int& count) {
	...
	saveTohistory("Movie added: " + titles[count]);
	saveMovieToFile(titles[count], genres[count], times[count]);
	count++;
...
void search() {
	...
	case 1: {
		...
		getline(cin, name);                //taking movie name as input using getline
		for (int i = 0;i < movieCount;i++) {
			if (name == movieTitle[i]) {      //searching movie name 
				cout << "\nYes Movie available at " << showTime[i] << " PM\n" << endl;
				found = true;
				break;
			}
		}
		...
		if (!found) {
			cout << "\nSorry! could not find a movie with this name.\n";
		}
		break;
	...
	case 2: {
		...
		for (int i = 0; i < movieCount; i++) {
			...
			if (!found)
				cout << "\nSorry could not find a movie with this Genre! Please Enter 1 to add this movie.\n" << endl;
		}
	}
		  break;
		  //Search by Showtime
	case 3: {
		cout << "\nEnter movie show time: ";
		cin >> time;
		found = false;
		for (int i = 0; i < movieCount; i++) {
			if (time == showTime[i]) {
				cout << "Yes the Movie " << movieTitle[i] << " is available for the show time " << time << "PM\n" << endl;
				found = true;
			}
		}
		if (!found) {
			cout << "\nSorry could not find a movie with this show time\n" << endl;
		}
		break;
	}
	default: {
...
void bookTickets() {
	...
	cout << "\nPlease enter you Name for reservation: ";
	...
	for (int i = 0;i < movieCount;i++) {
		if (movieName == movieTitle[i]) {                      //checking whether this movie is available
			...
			if (availableSeats > 0) {
				cout << "\nTicket booked Successfully under the name " << yourName << "\n" << endl;
				bookedSeats[i]++;
	...
	if (found) {
		saveTohistory("User " + yourName + " booked a ticket for " + movieName);  //Saving action to history.txt & reservation.txt
...
void viewReservations() {
	...
	cout << "\nEnter the movie name to view reservations: "; //asking viewers to enter movie name to view reservations for 
	getline(cin, movieName);
...
void deleteMovie() {
	...
	cout << "\nEnter the Movie name to delete it from the list: ";  //taking input from the user to delete the movie
	getline(cin, name);
	...
	for (int i = 0; i < movieCount; i++) {
		if (movieTitle[i] == name) {  //checking if the movie is in the list
			...
			for (int j = i;j < movieCount;j++) {
				...
				bookedSeats[j] = bookedSeats[j + 1];            //increasing the number of booked seats as the movie has been deleted
			...
			cout << "\nThe movie " << name << " was deleted successfully! " << endl;
			break;
		}
	}
	if (!found) {
		cout << "\nCould not find the movie \"" << name << "\" in the list.\n";  //error msg if movie not found in the list
	}
	if (found) {
...
int main() {
	...
	if (usertype == 1) {
		...
		else {   //if incorrect password show error message
			cout << "\nIncorrect Password!\n";
		}
	}
	else if (usertype == 2) {
